...
- Consensus fast-path: before calling the judge, if every surviving proposer normalizes to the same *short* answer (a crisp factual/numeric result), the judge is skipped and the agreed answer is returned directly - synthesis would only echo it. Triggers on non-streaming requests only; long, prose, or divergent answers always go through the judge. On by default (`config.consensusFastPath`).
- Fast-tier token cap (`config.fastTierMaxTokens`, 0 = off): caps `max_tokens` on the fast (non-escalate) proposers only. They are drafts for the judge / consensus check, so a tight cap makes them finish sooner; the deep tier and the judge stay uncapped, so frontier quality is unaffected. Only applies in escalation mode (non-empty `escalateModels`).
- Tiered fan-out / escalation (`config.escalateModels`): models listed here are *deferred* to a second phase. Phase 1 runs only the fast (non-escalate) proposers; if they reach consensus the deep models and judge never run, so an easy question never pays the slow reasoner's wall-clock. Only when the fast tier disagrees does fusion escalate - run the deep models and synthesize over everything. Empty list = single-phase (current behavior). The win concentrates on short/objective answers; long/open-ended prompts never reach consensus and always escalate, which is correct.
- Proposer timeout: `config.proposerTimeoutMs` (0 = disabled) drops a proposer that hangs past the limit so one slow model can't stall the fan-out; the judge then synthesizes from the survivors (partial-failure handling). The judge prompt instructs the model to compare candidates *silently* and emit only the final answer (no preamble or model mentions), so weaker/faster judge models stay clean.
- Config (stored in the `combos.config` JSON column): `judgeModel`, optional `judgePrompt` (a sensible default ships built-in), `showProvenanceFooter` (appends a per-model summary + coarse confidence to non-streaming answers; off by default), `consensusFastPath` (default on), and `proposerTimeoutMs` (default off). All are editable from the combos dashboard UI.
- Per-model status, latency, and cost are logged to request detail. Pure helpers in `fusion.js` (prompt builder, answer extraction, usage aggregation, outcome decision, footer, consensus detection, timeout race) are unit-tested under `tests/unit/fusion-*.test.js`.
